 ...
 * @author 033234581
 */
class Cliente_generico_model extends CI_Model{
    //put your code here
    ...
    public function buscaEmpresaPorCNPJ($cnpj){
        if($cnpj == null){throw new Exception("(Cliente_empresa_model) metodo buscaPorEmailECNPJ com parametros nulos");}
        $bdgerente = $this->load->database('gerencia', true);
    ...
    public function alterarClienteEmpresaGerencia($cliente){
        $bdgerente = $this->load->database('gerencia', true);
    ...
    public function inserirNovoCliente($cliente){
        if($cliente == null){
            throw new Exception("(Cliente_empresa_model) metodo inserirNovoCliente com parametros nulos");
        }
        $bdgerente = $this->load->database('gerencia', true);
        $cliente["senha_cliente"] = md5($cliente["senha_cliente"]);
        $resultado = $bdgerente->insert("cliente",$cliente);  
        if($bdgerente->affected_rows() == 0){throw new Exception("(Cliente_empresa_model) metodo inserirNovoCliente não alterou nenhuma linha");}
        return $resultado; 
    }
}
