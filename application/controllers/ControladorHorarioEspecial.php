...
class ControladorHorarioEspecial extends CI_Controller{
    ...
    public function manterHorarioEspecial(){
        ...
        try{
            ...
            $this->load->view("produto/ViewManterHorarioEspecial.php",$dados);
        }catch (Exception $e){
...
//            }
            ...
            if($fim < $ini){            
                ...
                $Horario1 = array(
                    ...
                    "empresa_horario_especial" => $empresaLogada["codigo_empresa"],
            ...
            for($i=0; $i<count($horario); $i++){
                ...
                if($horario[$i]["codigo_horario_especial"] == $codigo){
                    if($fim == $temp){                   
                        ...
                        if( !($this->HorariosEmpresa->removerHorarioEspecial($codigo, $empresaLogada["codigo_empresa"] )) ){
                            throw new Exception("Erro ao excluir dados.");
                        } 
                        if($codigo1 != 0 && !($this->HorariosEmpresa->removerHorarioEspecial($codigo1, $empresaLogada["codigo_empresa"] )) ){
                            throw new Exception("Erro ao excluir dados.");
                        } 
                    }else{
                        if( !($this->HorariosEmpresa->removerHorarioEspecial($codigo, $empresaLogada["codigo_empresa"] )) ){
    ...
    public function buscarHorarioEspecialPorID(){
        ...
        try{
            ...
            $json = json_decode( $meuPost );
            $codigo = $json->{"codigo_horario_especial"};
            $hora = $this->HorariosEmpresa->consultaHorarioEspecialPorCodigo($codigo, $empresaLogada["codigo_empresa"]); 
            $horario = $this->HorariosEmpresa->consultaHorarioEspecial($empresaLogada);
