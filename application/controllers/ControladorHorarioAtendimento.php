...
class ControladorHorarioAtendimento extends CI_Controller{
    ...
    public function manterHorarioAtendimento(){
        ...
        try{
            $empresaLogada = $this->session->userdata("empresa_logada");
            $horario = $this->HorariosEmpresa->consultaHorarioAtendimento($empresaLogada["codigo_empresa"]);
            $remov = array();
            $todos = array();
            ...
            $this->load->view("produto/ViewManterHorarioAtendimento.php",$dados); 
        }catch (Exception $e){
    ...
    public function buscarHorarioAtendimentoPorID(){
        ...
        try{
            ...
            $hora = $this->HorariosEmpresa->consultaHorarioAtendimentoPorCodigo($codigo, $empresaLogada["codigo_empresa"]); 
            $horario = $this->HorariosEmpresa->consultaHorarioAtendimento($empresaLogada["codigo_empresa"]);
            ...
            for($i=0; $i<count($horario); $i++){
                ...
                if($horario[$i]["codigo_horario_atendimento"] == $codigo){
                    if($fim == $temp){
                        ...
                        if( !($this->HorariosEmpresa->removerHorarioAtendimento($codigo, $empresaLogada["codigo_empresa"] )) ){
                            throw new Exception("Erro ao excluir dados.");
                        } 
                        if($codigo1 != 0 && !($this->HorariosEmpresa->removerHorarioAtendimento($codigo1, $empresaLogada["codigo_empresa"] )) ){
                            throw new Exception("Erro ao excluir dados.");
                        } 
                    }else{
                        if( !($this->HorariosEmpresa->removerHorarioAtendimento($codigo, $empresaLogada["codigo_empresa"] )) ){
    ...
    public function alterarHorarioAtendimento(){
        ...
        try{
           ...
           // $ini = date_format(date_create($json->{"inicio_horario_atendimento"}), 'H:i:s');
            ...
            $todosHorario = $this->HorariosEmpresa->consultaHorarioAtendimento($empresaLogada["codigo_empresa"]);
            $horarioDia = $this->HorariosEmpresa->consultaHorarioAtendimentoPorCodigo($codigo, $empresaLogada["codigo_empresa"]);
            ...
            if($codigo1 != 0){//se possui duas linhas
                if($fim < $ini){//verifico se ainda vai ficar as duas linhas se sim
                    $HorarioNovo1 = array(
                        ...
                        "empresa_horario_atendimento" => $empresaLogada["codigo_empresa"]     
                    );
