    ...
    <html>
        ...
        <body>
            ...
            <div class="container main2" ng-app="myApp" ng-controller="ControladorProduto">
                <div class="row">
                    <div class="col s12">
                        <h4><br/>Formas de pagamento</h4>
                        <!-- usado para pesquisar a tabela atual -->
                        <div class="input-field col s8 nopad">
                            <input type="text" ng-model="search" class="form-control" placeholder="Pesquisar formas de pagamento..." />
                        </div>
                    </div>
                    <br>
                    <div class="sorter"><h6 style="display: inline-block;">Ordenar por: </h6>
                        <a  ng-class="{'activetab  waves-light' : predicate === 'descricao_forma_pagamento',  'disabled': predicate !== 'descricao_forma_pagamento'}"
                            ng-click="predicate = 'descricao_forma_pagamento'; reverse = !reverse" class="btn">Descricao
                            <i ng-show="predicate === 'descricao_forma_pagamento' && !reverse" class="material-icons right">expand_more</i>
                            <i ng-show="predicate === 'descricao_forma_pagamento' && reverse" class="material-icons right">expand_less</i></a>
                    </div>
                    ...
                    <table class="table hoverable bordered">
                        <thead>
                            <tr>
                                <th ng-class="{'activetab waves-effect waves-light' : predicate === 'descricao_forma_pagamento'}"  ng-click="predicate = 'descricao_forma_pagamento';
                                            reverse = !reverse">Descrição
                                    <i ng-show="predicate === 'descricao_forma_pagamento' && !reverse" class="material-icons right">expand_more</i>
                                    <i ng-show="predicate === 'descricao_forma_pagamento' && reverse" class="material-icons right">expand_less</i>
                                </th>                                
                        ...
                        <tbody>
                            <tr dir-paginate="produto in produtos | filter:search:buscaSemAcento | filter:filters | orderBy:predicate:reverse:localeSensitiveComparator | itemsPerPage: resultlimit ">                              
                                <td> {{produto.descricao_forma_pagamento}} </td>                       
                                <td class="text-align-center" >
                                    <a ng-click="carregarProduto(produto.codigo_forma_pagamento)" title="alterar forma de pagamento" class="waves-light btn customAction" ><i class="material-icons centered">edit</i></a>
                                    <a ng-hide="{{produto.codigo_forma_pagamento == 1 }}" title="excluir forma de pagamento"  ng-click="excluirProduto(produto.codigo_forma_pagamento)" class="waves-light btn customAction"><i class="material-icons centered">delete</i></a>
                                </td>    
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <form name="produtoForm" id="modal-produto-form" class="modal" novalidate>
                        <div class="modal-content">
                            <h4 id="modal-produto-title">Inserir Forma de Pagamento</h4>
                            <div class="row">
                                <div class="input-field col s12">
                                    <input type="text" ng-model="descricaoInput" name="descricaoInput" id="descricaoInput" required aria-required="true" ng-pattern="/^[A-Za-záàâãéèêíïóôõöúçñÁÀÂÃÉÈÍÏÓÔÕÖÚÇÑ ]+([0-9,.'-()]+)?/" placeholder="Nome da forma de pagamento"/>
                    ...
                    <div class="fixed-action-btn" style="bottom:45px; right:24px;">
                        <a class="waves-effect waves-light btn modal-trigger btn-floating btn-large red" href="#modal-produto-form" title="inserir forma de pagamento" ng-click="showCreateForm()"><i class="large material-icons">add</i></a
                    </div> <!-- END BUTTON -->
...
<script>
    ...
    app.controller('ControladorProduto', function ($scope, $http) {
        // read produtos
        $scope.produtos = <?php echo json_encode($formas) ?>;
       ...
       $scope.localeSensitiveComparator = function(v1, v2) {
        ...
        $scope.showCreateForm = function () {
            ...
            $scope.clearForm();
            // change modal title
            $('#modal-produto-title').text("Inserir Forma de pagamento");
        ...
        $scope.incluirProduto = function () {
            ...
            $http.post('<?=base_url("index.php/ControladorFormaPagamento/incluirFormaDePagamento")?>', {
                'descricao_forma_pagamento': $scope.descricaoInput
            }
        ...
        $scope.carregarProduto = function (id) {
            // change modal title
            $('#modal-produto-title').text("Alterar Forma de pagamento");
            // show udpate produto button
            $('#btn-update-produto').show();
            // show create produto button
            $('#btn-create-produto').hide();
            // post id of produto to be edited
            $http.post('<?=base_url("index.php/ControladorFormaPagamento/buscarFormaDePagamentoPorID")?>', {
                'codigo_forma_pagamento': id
            }).success(function (data, status, headers, config) {
                        // put the values in form
                        $scope.codigo_forma_pagamento = data["codigo_forma_pagamento"];
                        $scope.descricaoInput = data["descricao_forma_pagamento"];
                        // show modal
        ...
        $scope.alterarProduto = function () {
            ...
            $http.post('<?=base_url("index.php/ControladorFormaPagamento/confirmarAlterarFormaDePagamento")?>', {
                'codigo_forma_pagamento': $scope.codigo_forma_pagamento,
                'descricao_forma_pagamento': $scope.descricaoInput
            }).success(function (data, status, headers, config) {      
                        ...
                        for (numero=0; numero < $scope.produtos.length; numero++){
                            if(parseInt($scope.produtos[numero].codigo_forma_pagamento) === parseInt($scope.codigo_forma_pagamento)) {
                                $scope.produtos[numero].descricao_forma_pagamento = $scope.descricaoInput;
                            }
        ...
        $scope.excluirProduto = function (codigo) {
            if (confirm("Tem certeza que deseja excluir?")) {                                                                                                                 
                $http.post('<?=base_url("index.php/ControladorFormaPagamento/confirmarRemoverFormaDePagamento")?>', {
                    'codigo_forma_pagamento': codigo
                }).success(function (data, status, headers, config) {
                    Materialize.toast("Dados excluídos com sucesso", 4000);                                           
                    for (numero=0; numero < $scope.produtos.length; numero++){
                        if($scope.produtos[numero].codigo_forma_pagamento === codigo) {
